...
const colorRegex =
/(#([0-9a-f]{3}){1,2}|(rgba|hsla)\(\d{1,3}%?(,\s?\d{1,3}%?){2},\s?(1|0?\.\d+)\)|(rgb|hsl)\(\d{1,3}%?(,\s?\d{1,3}%?\)){2})/ig;
...
function parse(data) {
  const { stylesheet: { rules } } = parseCss(data);
  ...
  colorRules.forEach(({ selectors, properties }) => {
    properties.forEach(({ property, value, originalValue }) => {
      ...
      selectors = selectors.filter(selector => ignoredSelector.indexOf(selector) === -1);
...
function processRules({ declarations, rules, type, selectors }) {
  ...
  const properties = declarations.map(({ value, property }) => {
